...
const TOOLS: ToolDef[] = [
  { name: "index_project", description: "Explicitly refresh the asset index. Default is incremental; force=true rebuilds for guaranteed freshness.", schema: { path: z.string().optional(), force: z.boolean().optional() } },
  { name: "index_status", description: "Stored index location, counts, and last-indexed time; does not scan live assets or prove freshness.", schema: {} },
  ...
  { name: "get_addressable_info", description: "Addressables membership, group, labels, references, and reachability for one asset or address.", schema: { asset: z.string().describe("asset path, exact name, guid, or Addressable address") } },
  {
  ...
  { name: "list_addressable_groups", description: "Addressables group inventory with entry counts, indexed source bytes, and labels.", schema: {} },
